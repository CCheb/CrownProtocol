...
public partial class NetworkCore : MultiplayerSpawner
{
    ...
    private Godot.Collections.Array<int> FindBadObjectsWith(long peerId)
    {
        ...
        foreach (var i in GenericCore.Instance.netObjects.Keys)
        {
            if (GenericCore.Instance.netObjects[i].OwnerId != peerId) continue;
            badObjs.Add(i);
        }
    ...
    {
        GenericCore.Instance.netObjects[badObj].GetParent().QueueFree();
        GenericCore.Instance.netObjects.Remove(badObj);
    }
    // Destroys a single NetID from the list
    public void NetDestroyObject(NetID netId)
    {
        if(netId.myNetworkCore == null)
        {
            try
            {
                GenericCore.Instance.netObjects.Remove((int)netId.netObjectID);
        ...
        foreach (var i in GenericCore.Instance.netObjects.Keys)
        {
            try
            {
                if (GenericCore.Instance.netObjects[i] != netId) continue;
                Discard(i);
            }
            catch
            {
